...
const FeedbackCard = ({ title, text, avatar, name, position }) => {
  ...
  return (
    <div className={`border w-94 h-87.5 p-6 transition-all duration-300 hover:shadow-xl hover:scale-105 ${
      darkMode 
        ? 'border-green-400 hover:border-green-400' 
        : 'border-[#69B99D] md:border-[#DEDCDA] hover:border-[#69B99D]'
    }`}>
      <FaQuoteLeft size={42} className={`transition-colors duration-300 ${
        darkMode ? 'text-gray-700' : 'text-gray-100'
      }`} />
      <h2 className={`w-66 text-lg font-semibold mt-6 transition-colors duration-300 ${
        darkMode ? 'text-white' : 'text-[#1F2744]'
      }`}>{title}</h2>
      <p className={`w-82 mt-4 h-24 transition-colors duration-300 ${
